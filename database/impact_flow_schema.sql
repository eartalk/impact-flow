...
CREATE TABLE IF NOT EXISTS project (
  ...
  pending_commit_count INT NOT NULL DEFAULT 0 COMMENT '当前待检测合并数量（字段名为历史兼容保留）',
  pending_commits JSON NULL COMMENT '当前待检测合并摘要列表（JSON，字段名为历史兼容保留）',
...
CREATE TABLE IF NOT EXISTS project_inspection_log (
  ...
  pending_commit_count INT NOT NULL DEFAULT 0 COMMENT '本次巡检发现的待检测合并数量（字段名为历史兼容保留）',
...
CREATE TABLE IF NOT EXISTS ai_provider_config (
  ...
  max_files INT NOT NULL DEFAULT 80 COMMENT '单个AI分析批次最大文件数量',
  max_symbols INT NOT NULL DEFAULT 50 COMMENT '单个AI分析批次最大Symbol数量',
  created_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
